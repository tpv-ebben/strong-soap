...
DefinitionsElement.prototype.addChild = function(child) {
  ...
  if (child instanceof TypesElement) {
    ...
    schemaElement.addChild(child);
    child.parent = schemaElement;
  }
...
SimpleTypeElement.prototype.description = function(definitions) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    if (child instanceof RestrictionElement) {
      this._description = this.$name + "|" + child.description();
      return this._description;
    }
  }
  return this._description;
...
ComplexTypeElement.prototype.description = function(definitions, xmlns) {
  ...
  this._description = '*'; // In describing state
  var children = this.children || [];
  for (var i = 0, child; child = children[i]; i++) {
    if (child instanceof ChoiceElement ||
      ...
      this.emit('described', this._description);
      return this._description;
    }
  }
  this._description = {};
...
ElementElement.prototype.description = function(definitions, xmlns) {
  ...
  if (type) {
    ...
    if (typeElement && !(typeName in Primitives)) {
      ...
      if (description === '*') {
        var self = this;
        typeElement.once('described', function() {
          if (self.$ref) {
            self._description = typeElement._description;
          } else {
            self._description[name] = typeElement._description;
          }
        });
      } else {
        if (this.$ref) {
    ...
    var children = this.children;
    element[name] = {};
    var factory = function setDescription(child) {
      return function() {
        self._description[name] = child._description;
      };
    };
    for (var i = 0, child; child = children[i]; i++) {
      if (child instanceof ComplexTypeElement ||
        ...
        if (childDesc === '*') {
          child.once('described', factory(child));
        } else {
...
WSDL.prototype.findSchemaType = function(name, nsURI) {
  ...
  var schema = this.definitions.schemas[nsURI];
  if (!schema || !schema.complexTypes || !schema.types) {
    return null;
  }

  return schema.complexTypes && schema.complexTypes[name] ||
    schema.types && schema.types[name];
};
...
WSDL.prototype.findChildSchemaObject = function(parameterTypeObj, childName) {
  ...
  if (object.$type) {
    ...
    if (this.options.ignoredNamespaces.indexOf(typeInfo.prefix) === -1) {
      if (typeInfo.prefix === TNS_PREFIX) {
        childNsURI = parameterTypeObj.$targetNamespace;
      } else {
        if (!childNsURI && this.definitions.schemas) {
          // First check target schema
          var schema = this.definitions.schemas[parameterTypeObj.$targetNamespace];
          if (schema) {
            childNsURI = schema.xmlns[typeInfo.prefix];
          }
        }
        if (!childNsURI) {
          // Fall back to definition level namespace mapping
